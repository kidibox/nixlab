...
in
{
  options.nixlab.k3s = {
    ...
    bootstrap = mkEnableOption "bootstrap";
  };
  ...
  config = mkIf cfg.enable {
    services.k3s = {
      ...
      extraFlags = toString [
        ...
        "--token=foo"
        "--tls-san=10.0.100.189,k3s.kidibox.net"
    ...
    # system.activationScripts.k3s = mkIf cfg.bootstrap ''
    ...
    services.keepalived = {
      ...
      vrrpScripts.apiserver = {
        script = "${pkgs.kubectl}/bin/kubectl --kubeconfig=/etc/rancher/k3s/k3s.yaml get --raw=/readyz";
        user = "root";
        interval = 10;
        rise = 1;
        fall = 1;
      };
      vrrpInstances.apiserver = {
        interface = "enp6s18";
        virtualRouterId = 189;
        trackScripts = [ "apiserver" ];
        virtualIps = [
          { addr = "10.0.100.189/24"; }
        ];
      };
    };
  };
